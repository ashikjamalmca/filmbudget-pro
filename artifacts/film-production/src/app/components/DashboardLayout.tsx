...
export function DashboardLayout({ children, currentPage, onNavigate, onLogout }: DashboardLayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
      {/* Desktop Sidebar */}
      <div 
        className={`hidden lg:flex bg-[#1E3A8A] text-white transition-all duration-300 ${
          collapsed ? 'w-16' : 'w-64'
        } flex-col`}
      >
        {/* Logo */}
        <div className="p-4 flex items-center justify-between border-b border-white/10">
          {!collapsed && <span className="text-xl">FilmBudget Pro</span>}
          <Button
            variant="ghost"
            size="icon"
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="bg-white border-b border-gray-200 px-4 lg:px-8 py-3 lg:py-4 flex items-center justify-between gap-4">
          ...
          <Sheet open={mobileMenuOpen} onOpenChange={setMobileMenuOpen}>
            ...
            <SheetContent side="left" className="p-0 w-64 bg-[#1E3A8A] text-white">
              <SheetHeader className="p-4 border-b border-white/10">
                <SheetTitle className="text-white text-left">FilmBudget Pro</SheetTitle>
              </SheetHeader>
          ...
          <div className="flex-1 min-w-0">
            <h1 className="text-lg lg:text-2xl text-gray-900 truncate">Lokah</h1>
            <p className="text-xs lg:text-sm text-gray-600 truncate">Jan 2025 - Mar 2025 • ₹50,00,000</p>
          </div>
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" className="flex items-center gap-2 shrink-0">
                ...
                <span className="hidden md:inline">John Producer</span>
              </Button>
            </DropdownMenuTrigger>
